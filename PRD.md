...
- **Framework**: Vite with React & TypeScript (`react-router-dom` using `HashRouter` to natively prevent 404s on static hosts).
...
- **Link Expiry Limits**: Users may optionall provide a `Max Clicks Limit` when generating a link. Upon interception, the Redirection flow evaluates if the limit has been hit, throwing a stylized "Link Expired" exception rather than redirecting.
...
- **Login / Register Functionality**: New users can register and immediately log in. The application bounces un-authenticated users back to login cleanly.
